...
define(['util'], function(){
	...
	*/
	...
	WmNavBar.prototype.bindTabItemAction = function(index,action){
		...
		$(liEle[index]).tap(tapAction,liEle[index]);
	};
	/*选中标签页*/
	...
	WmNavBar.prototype.initTabGroup = function(){
		...
		for(var i=0;i<tip.length;i++){
			$(tip[i]).tap(function(ele){
				var fold = $(ele).find('span');
				var option = $(ele).parent().find('div.option');
	...
	WmNavBar.prototype.bindTabGroupItemAction = function(groupIndex,itemIndex,action){
		var tapAction = function(p){
			$(p[0].parentNode.parentNode).css("display","none");
			var fold = $(p[0].parentNode.parentNode).prev().find('span');
			$(fold[0]).removeClass("e_ico-fold").addClass("e_ico-unfold");
			if(typeof action =="function"){
				action();
			}else{
		...
		if(groupIndex < tabGroup.length){
			...
			if(itemIndex < lis.length){
				$(lis[itemIndex]).tap(tapAction,$(lis[itemIndex]));
			}
		}
	};
	/*增加Option*/
